...
struct vary_node * find_node(struct vary_node* root, char* name){
...
struct vary_node ** second_pass() {
  ...
  for (i = 0; i < lastop; i++){
	...
	exit(0);
      ...
      count = op[i].op.vary.start_val;
      num_steps = op[i].op.vary.end_frame - op[i].op.vary.start_frame;
      step = (op[i].op.vary.end_val - op[i].op.vary.start_val) / num_steps;
      for (j = op[i].op.vary.start_frame; j <= op[i].op.vary.end_frame; j++){
	if (find_node(knobs[j], op[i].op.vary.p -> name) == NULL){
	  knob = (struct vary_node*)malloc(sizeof(struct vary_node*));
	  strcpy(knob -> name, op[i].op.vary.p -> name);
	  knob -> value = count;
	  knob -> next = NULL;
	  knobs[j] -> next = knob;
	  knob_names[a] = knob -> name;
	}
	count += step;
      }
    }
...
void my_main( int polygons ) {
  ...
  if (num_frames == 0){
	...
	case DISPLAY:
      ...
      sprintf(frame_name, "%s%03d.png", name, i);
